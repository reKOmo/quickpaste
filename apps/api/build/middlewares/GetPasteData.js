...
function getPasteData(req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        const pastes = yield db.query("SELECT pastes.*, users.username AS owner_username FROM pastes LEFT JOIN users ON users.id = pastes.owner_id WHERE pastes.uuid = $1;", [req.additional.pasteUUID]);
        ...
        yield db.query("UPDATE pastes SET last_visited = NOW() WHERE id = $1;", [pastes.rows[0].id]);
        next();
    });
}
