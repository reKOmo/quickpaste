...
<script setup>
    ...
    let {data: paste, error} = await useAsyncData("paste", async () => {
    ...
    let {data: err} = await useAsyncData("error", () => !paste.value && error.value ? error.value.statusCode : 200, { server: true });
